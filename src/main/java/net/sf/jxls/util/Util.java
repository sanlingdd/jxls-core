...
public final class Util {
		...
		for (int i = 0, c = rowCollection.getDependentRowNumber(); i <= c; i++) {
			...
			if (hssfRow.getFirstCellNum() >= 0 && hssfRow.getLastCellNum() >= 0) {
				for (int j = hssfRow.getFirstCellNum(), c2 = hssfRow.getLastCellNum(); j <= c2; j++) {
					...
					removeRowCollectionPropertyFromCell(cell,
							rowCollection.getCollectionProperty().getFullCollectionName());
				}
			}
		}
	}
	...
	private static void removeRowCollectionPropertyFromCell(org.apache.poi.ss.usermodel.Cell cell,
		...
		if (cell != null && cell.getCellType() == org.apache.poi.ss.usermodel.Cell.CELL_TYPE_STRING) {
			...
			cell.setCellValue(cell.getSheet().getWorkbook().getCreationHelper()
					.createRichTextString(cellValue.replaceAll(strToReplace, "")));
		}
	}
	...
	private static void prepareCollectionPropertyInCellForDuplication(org.apache.poi.ss.usermodel.Cell cell,
			String collectionName, String collectionItemName) {
		if (cell != null && cell.getCellType() == org.apache.poi.ss.usermodel.Cell.CELL_TYPE_STRING) {
			String cellValue = cell.getRichStringCellValue().getString();
	...
	public static void prepareCollectionPropertyInRowForContentDuplication(RowCollection rowCollection) {
		for (int i = 0, c = rowCollection.getCells().size(); i < c; i++) {
			...
			prepareCollectionPropertyInCellForDuplication(cell.getPoiCell(),
					rowCollection.getCollectionProperty().getFullCollectionName(),
					rowCollection.getCollectionItemName());
		}
	}
		...
		if (row.getCells().size() > rowCollection.getCells().size()) {
			for (int i = 0; i < row.getCells().size(); i++) {
				...
				if (!rowCollection.containsCell(cell)) {
					shiftColumnUp(cell,
							row.getPoiRow().getRowNum() + rowCollection.getCollectionProperty().getCollection().size(),
	...
	public static void copySheets(Sheet newSheet, Sheet sheet, String expressionToReplace,
	...
	public static void copyCell(org.apache.poi.ss.usermodel.Cell oldCell, org.apache.poi.ss.usermodel.Cell newCell,
			boolean copyStyle, String expressionToReplace, String expressionReplacement) {
		if (copyStyle) {
			newCell.setCellStyle(oldCell.getCellStyle());
			copyConditionalFormat(oldCell, newCell);
		}
	...
	static String replaceExpressions(String originalExpression, String expressionToReplace,
			String expressionReplacement) {
		/*
		 * assuming that: - originalExpression may contain expressions of the
		 * form ${expr} - expressionToReplace is an identifier (alphanumeric +
		 * underscore) - expressionReplacement is an identifier too - only root
		 * value may be replaced
		 */
		return originalExpression.replaceAll("(\\$\\{)" + expressionToReplace + "((\\b.*)?\\})",
	...
	public static void copyConditionalFormat(org.apache.poi.ss.usermodel.Cell oldCell,
		...
		if (!rules.isEmpty()) {
			ncf.addConditionalFormatting(
					new CellRangeAddress[] { new CellRangeAddress(newCell.getRowIndex(), newCell.getRowIndex(),
							newCell.getColumnIndex(), newCell.getColumnIndex()) },
					rules.toArray(new ConditionalFormattingRule[0]));
		}
	}
	...
	public static void setPrintArea(Workbook resultWorkbook, int sheetNum) {
		int maxColumnNum = 0;
		for (int j = resultWorkbook.getSheetAt(sheetNum).getFirstRowNum(), c = resultWorkbook.getSheetAt(sheetNum)
				.getLastRowNum(); j <= c; j++) {
